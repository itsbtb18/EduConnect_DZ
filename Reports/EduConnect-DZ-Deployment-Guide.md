# Madrassa DZ — Complete Deployment & Hosting Guide

> This document covers everything you need to know to host, deploy, and maintain Madrassa DZ in production — including the web admin panel, Django backend, Flutter mobile app, and all associated costs. No prior DevOps experience required.
...
Madrassa DZ consists of three separate things that all need to be deployed:
...
### Route 53
AWS's DNS service. It connects your domain name (e.g., madrassa.dz) to your EC2 server's IP address. When someone types your domain, Route 53 tells their browser where to go.
...
You need a domain name for your application (e.g., `madrassadz.com` or `madrassa.dz`).
...
**Domain suggestions:**
- `madrassadz.com` — broadly recognized
- `madrassa.dz` — Algerian country-code domain, more local trust but harder to register (requires Algerian business paperwork)
...
**Recommendation for Madrassa DZ:** Skip iOS initially. The vast majority of Algerian users are on Android. Start with direct APK or Play Store, generate revenue, then invest in iOS development and the $99/year fee.
...
Madrassa DZ requires three deployed components: the Django backend on EC2, the React frontend on S3/CloudFront, and the Flutter mobile app distributed directly or through Google Play. The Flutter app talks to the same backend as the web panel — no separate server needed.
...
*Document prepared for Madrassa DZ — Internal deployment reference.*
