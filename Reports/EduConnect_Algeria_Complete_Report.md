# 📚 ILMI — Complete Product Conception & Technical Blueprint
...
ILMI is built as a **Multi-Tenant SaaS (Software as a Service)** platform. This means you deploy the system **ONE time** in the cloud, and **ALL schools** use the exact same system. You never redeploy for each new school.
...
| **ILMI** | Hundreds of Algerian schools → one system |
...
```
Base URL: https://api.ilmi.dz/api/v1/
```
...
Create a **Postman Collection** called "ILMI API" with folders: Auth, Grades, Homework, Chat, Chatbot, etc. For each endpoint, save:
